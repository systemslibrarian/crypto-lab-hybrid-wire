 ...
 * element and owns no text node. The repo already carried a 1.4.11 check —
 * `e2e/border-contrast.spec.ts` — and it measured `.input` and nothing else,
 * which is exactly one of the two selectors `--border` was correctly applied to.
 * A check aimed only where a rule is already kept cannot fail. What it never
 * looked at was the pair of `role="switch"` resilience toggles, whose track and
 * thumb are hardcoded `rgba()` values appearing in no token and in no theme
 * block, and which are the only thing on the page carrying the switch's
 * on/off STATE.
 *
 ...
 *    background really is painted underneath the border. Every panel and every
 *    control on this page has a translucent fill, so getting that backdrop
 *    wrong would move most of the numbers this oracle reports.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * Load-bearing here: `body` carries two `radial-gradient(circle at …)`
     * washes over `--bg`, and every panel above them is translucent, so the
     * gradient shows through to whatever is being measured. The active tab
     * button is itself a two-stop `linear-gradient`, judged at the sample point
     * rather than at its worst stop.
     */
