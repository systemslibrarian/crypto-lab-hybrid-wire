 ...
 *
 *  - text over a surface axe declines to resolve. Nearly every surface on this
 *    page is one. `body` paints two `radial-gradient()` washes over `--bg`, and
 *    then every panel above them is TRANSLUCENT: `--surface` is
 *    `rgba(8,15,31,0.88)`, `--surface-raised` `rgba(15,23,42,0.92)`,
 *    `--surface-sunken` `rgba(6,12,24,0.9)`, and the light theme is the same
 *    shape in white. On top of that `.cl-hero-why` is a
 *    `color-mix(in oklab, var(--accent-blue) 6%, transparent)` and the active
 *    tab is a two-stop `linear-gradient`. Chromium hands all of those to
 *    `getComputedStyle` unresolved, so axe files the text on them under
 *    `incomplete` — which is to say a violations-only gate measured almost no
 *    text on this page at all.
 ...
 *    at `.85` and `.wire-broken` at `.4`. The second one matters: a wire the
 *    reader has just declared broken is deliberately dimmed, and the gate this
 *    replaces injected `opacity: 1 !important` over the whole document before
 *    every scan, so that state was only ever measured in a rendering nobody
 *    sees.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The clipping boxes
 *    here are `.table-card` (the four-column threat matrix, which becomes
 *    `overflow-x: auto` under 760px and is therefore clipped at 380px), the two
 *    `.kem-mini` `<pre>` diagrams and `.formula` (all `overflow-x: auto` around
 *    `white-space: pre` art), and the `overflow: hidden` wire-diagram wrapper.
 *    Content
 *    scrolled past the client box is not dimmed or partly drawn — it is absent
 *    from the frame, and asking what colour it sits on has no answer. Its rect is
 ...
 *    never laid down. Nothing on this page does that today; the rule is kept
 *    because it costs nothing and its absence invents a 1:1 failure the first
 *    time someone reaches for the idiom.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This is
 *    load-bearing here, not defensive. The handshake diagram is an `<svg>`, and
 *    each labelled `.wire-token` is a `<rect>` filled `--surface-raised`
 *    followed by a `<text>` filled `--text`. SVG has no `background-color`, so
 *    an ancestor-only walk would composite "X25519 secret" onto the svg's own
 *    transparent background — falling through to the page, or to WHITE — and
 *    report a ratio nothing on screen has. Any earlier sibling shape whose box
 *    contains the text box is composited first instead.
 *
 * ONE THING THIS WALK CANNOT SEE, stated so it is not mistaken for coverage.
 * `nontext.ts` runs alongside it and covers both halves: control boundaries
 * (SC 1.4.11), which the element walk cannot reach because a boundary is not
 * text, and `::before`/`::after` ink, which it cannot reach because a
 * pseudo-element is not an element. On this page that second oracle is what
 * measures the `.kem-aside > summary::before` disclosure triangle and the
 * `.combiner-op` arrows.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * matters" aside is `color-mix(in oklab, var(--accent-blue) 6%,
     * transparent)`, and Chromium reports that
     ...
     * fabricates failures (a dark-on-light wire card read as dark-on-dark) and, far
     ...
     *
     * The threat matrix alone is a four-column table of prose, the deployment
     * grid is six cards, and the drive scans after every one of ~28 steps; every
     * text node on the page walks the same handful of ancestors each time.
     ...
     * surface it happens to sit on and INVENTS failures. Live on this page: the
     * `.sr-only` labels for the sender `<select>` and the message `<input>`, and
     * the `.sr-only` paragraph describing the HKDF combiner for a screen reader,
     * are all real boxes that paint nothing. Deliberately narrow — only a
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page's one disclosure is `.kem-aside` ("What is a
      // KEM, and why does it differ from Diffie-Hellman?") on the two-wires tab,
      // whose open state is a real state the gate scans explicitly.
     ...
     * it, the resilience verdict's `aria-hidden` status emoji (⏳/🔒/🛡️/🔓) and
     * the misconception panel's ✕/⚠️ glyphs are measured as if each took its
     * element's inherited `color`, but an emoji paints in its own multicolour,
     * so that ratio describes nothing on screen.
     */
